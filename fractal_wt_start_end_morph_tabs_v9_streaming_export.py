...
# No fixed START→END export duration limit. Earlier versions clamped morph
# renders to 60 seconds, but that was an arbitrary guard rather than a DSP
# requirement. Long renders are still held in RAM before writing the WAV, so
# _render_length_summary reports warnings instead of silently shortening them.
MORPH_LONG_RENDER_WARNING_SECONDS = 60.0
MORPH_MEMORY_WARNING_BYTES = 1_000_000_000
MORPH_MEMORY_CAUTION_BYTES = 4_000_000_000
...
def _render_length_summary(mode: str, value: float, frequency: float, sample_rate: int, bit_depth: str) -> RenderSummary:
    ...
    minimum_samples = max(1, int(math.ceil(samples_per_cycle)))

    mode = str(mode or "Cycles")
    try:
        requested = float(value)
    except (TypeError, ValueError):
        requested = float(MORPH_DEFAULT_CYCLES)
    if not math.isfinite(requested):
        requested = float(MORPH_DEFAULT_CYCLES)

    if mode == "Seconds":
        # Deliberately no upper clamp: the user may want files minutes or hours long.
        seconds = max(float(requested), one_cycle_seconds)
        sample_count = max(minimum_samples, int(round(seconds * sr)))
    elif mode == "Samples":
        # Deliberately no upper clamp: sample count is accepted as requested,
        # subject only to the minimum of one nominal waveform cycle.
        sample_count = max(minimum_samples, int(round(max(1.0, float(requested)))))
    else:
        # Deliberately no upper clamp: cycle count is a sound-design unit, and
        # long files are useful as evolving drones or raw material.
        cycles = max(1.0, float(requested))
        sample_count = max(minimum_samples, int(round((cycles / freq) * sr)))

    duration = sample_count / float(sr)
    nominal_cycles = duration * freq

    # There is no fixed render-duration limit, but this renderer still builds
    # the whole mono buffer in memory before writing the WAV. Warn when the
    # requested render is likely to be slow or memory-heavy.
    working_bytes = sample_count * 20  # float32 render + float64 processing/writing, roughly.
    if duration >= MORPH_LONG_RENDER_WARNING_SECONDS:
        warnings.append(
            f"large render: {_format_seconds_compact(duration)}; render is held in RAM before WAV write"
        )
    if working_bytes >= MORPH_MEMORY_CAUTION_BYTES:
        warnings.append(f"very large memory use likely: working buffers roughly {working_bytes / 1_000_000_000:.2f} GB+")
    elif working_bytes >= MORPH_MEMORY_WARNING_BYTES:
        warnings.append(f"large memory use likely: working buffers roughly {working_bytes / 1_000_000_000:.2f} GB+")
...
# -----------------------------------------------------------------------------
# v9 streaming WAV export pass
#
# The v8 no-render-limit pass removed the arbitrary 60-second clamp, but the
# original export path still rendered the complete mono float buffer in RAM and
# only then wrote the WAV file. That is acceptable for short files, but it is the
# wrong shape for long drone/sample-material exports.
#
# This layer adds a streaming export path for cleanup modes that can be decided
# locally: Off, Per Cycle DC, and Per Cycle Peak normalisation. The defaults are
# Per Cycle + Per Cycle Peak, so the ordinary long-render case now renders one
# cycle or chunk, post-processes that local piece, writes PCM frames immediately,
# and moves on.
#
# Whole File DC removal and Whole File Peak normalisation are deliberately left
# on the older buffered path. Those modes need knowledge of the whole render's
# mean or peak. They could be made streaming with a two-pass render or a temporary
# raw-float spool file, but silently pretending they are one-pass streaming would
# change the audio result.
#
# Embedded-systems note: this decision is friendly to low-RAM targets because the
# default export path no longer needs enough memory for the whole WAV. It does not
# make long renders free on embedded systems: CPU time, flash/SD-card write wear,
# filesystem limits, and the classic RIFF/WAV 4 GiB size ceiling still matter.
# It is unlikely that this Tk/sounddevice desktop app will be used for hour-long
# exports on embedded hardware, but the render/write separation keeps that door
# more open than a full-buffer design.
# -----------------------------------------------------------------------------

STREAM_EXPORT_CHUNK_SAMPLES = 65_536
STREAM_WAV_FLUSH_BYTES = 16 * 1024 * 1024
RIFF_WAV_MAX_DATA_BYTES = (4 * 1024 * 1024 * 1024) - 64
RIFF_WAV_CAUTION_BYTES = int(RIFF_WAV_MAX_DATA_BYTES * 0.90)


def _normalise_mode_token(value: str) -> str:
    return str(value or "").strip().lower().replace("_", " ")


def _canonical_dc_mode(value: str) -> str:
    token = _normalise_mode_token(value)
    if token == "whole file":
        return "Whole File"
    if token == "per cycle":
        return "Per Cycle"
...
def _canonical_norm_mode(value: str) -> str:
    token = _normalise_mode_token(value)
    if token == "whole file peak":
        return "Whole File Peak"
    if token == "per cycle peak":
        return "Per Cycle Peak"
    return "Off"


def _wav_container_bytes_per_sample(bit_depth: str) -> int:
    label = str(bit_depth or "")
    if label.startswith("8"):
        return 1
    if label.startswith("24"):
        return 3
    if label.startswith("32"):
        return 4
    # 12-bit is intentionally stored in a 16-bit PCM container.
    return 2


def _render_cleanup_streamable(dc_mode: str, norm_mode: str, *, samples_per_cycle: float) -> tuple[bool, str]:
    """Return whether this cleanup choice can stream without changing semantics."""
    dc = _canonical_dc_mode(dc_mode)
    norm = _canonical_norm_mode(norm_mode)
    if dc == "Whole File" or norm == "Whole File Peak":
        return False, "Whole File cleanup needs whole-render mean/peak; using buffered render path"
    if (dc == "Per Cycle" or norm == "Per Cycle Peak") and float(samples_per_cycle) < 4.0:
        return False, "Per Cycle cleanup has fewer than 4 samples/cycle; using buffered legacy fallback"
    return True, "streaming path supports Off/Per Cycle cleanup"


def _render_length_summary(mode: str, value: float, frequency: float, sample_rate: int, bit_depth: str) -> RenderSummary:  # type: ignore[override]
    """No hard duration clamp; warn about long renders, container size, and preview memory.

    This v9 version replaces the v8 wording that implied every long export must
    be held in RAM. Default Per Cycle export now streams to disk. Whole File
    cleanup and audition preview still use full buffers.
    """
    warnings: list[str] = []
    suggestions: list[str] = []
    sr = max(1, int(sample_rate))
    ...
    duration = sample_count / float(sr)
    nominal_cycles = duration * freq
    pcm_bytes = int(sample_count) * _wav_container_bytes_per_sample(bit_depth)
    preview_working_bytes = sample_count * 20

    if duration >= MORPH_LONG_RENDER_WARNING_SECONDS:
        warnings.append(
            f"large render: {_format_seconds_compact(duration)}; default Per Cycle export streams to disk, but preview/Whole File cleanup may still need RAM"
        )
    if pcm_bytes >= RIFF_WAV_MAX_DATA_BYTES:
        warnings.append(
            f"classic RIFF/WAV data may exceed 4 GiB ({pcm_bytes / 1_000_000_000:.2f} GB); split the render or use a container with RF64/W64 support"
        )
    elif pcm_bytes >= RIFF_WAV_CAUTION_BYTES:
        warnings.append(
            f"classic RIFF/WAV data is near the 4 GiB limit ({pcm_bytes / 1_000_000_000:.2f} GB)"
        )
    if preview_working_bytes >= MORPH_MEMORY_CAUTION_BYTES:
        warnings.append(
            f"preview or Whole File cleanup may need roughly {preview_working_bytes / 1_000_000_000:.2f} GB+ of working RAM"
        )
    elif preview_working_bytes >= MORPH_MEMORY_WARNING_BYTES:
        warnings.append(
            f"preview or Whole File cleanup may need roughly {preview_working_bytes / 1_000_000_000:.2f} GB+ of working RAM"
        )
...
def _pcm_bytes_from_float_mono(audio: np.ndarray, bit_depth: str) -> bytes:
    """Encode a mono float chunk as PCM bytes, matching _write_wav_mono."""
    y = np.clip(np.asarray(audio, dtype=np.float64).reshape(-1), -1.0, 1.0)
    label = str(bit_depth or "")
    if label.startswith("8"):
        return np.clip(np.round((y * 127.0) + 128.0), 0, 255).astype(np.uint8).tobytes()
    if label.startswith("12"):
        q = np.round(y * 2047.0) / 2047.0
        return np.clip(np.round(q * 32767.0), -32768, 32767).astype("<i2").tobytes()
    if label.startswith("16"):
        return np.clip(np.round(y * 32767.0), -32768, 32767).astype("<i2").tobytes()
    if label.startswith("24"):
        vals = np.clip(np.round(y * 8388607.0), -8388608, 8388607).astype(np.int32)
        ...
        for v in vals:
            ...
            p += 3
        return bytes(b)
    return np.clip(np.round(y * 2147483647.0), -2147483648, 2147483647).astype("<i4").tobytes()


class _StreamingMonoWavWriter:
    """Chunked mono WAV writer for long exports.

    This writes ordinary chunks through the Python file buffer and periodically
    flushes that buffer. It deliberately does not fsync every chunk; fsync would
    make long renders much slower and is not needed for normal export behaviour.
    """

    def __init__(self, path: str, *, sample_rate: int, bit_depth: str, total_frames: int):
        self.path = path
        self.sample_rate = int(sample_rate)
        self.bit_depth = str(bit_depth)
        self.total_frames = max(0, int(total_frames))
        self.raw = open(path, "wb")
        self.wf = _wave.open(self.raw, "wb")
        self.wf.setnchannels(1)
        self.wf.setframerate(self.sample_rate)
        if self.bit_depth.startswith("8"):
            sampwidth = 1
        elif self.bit_depth.startswith("24"):
            sampwidth = 3
        elif self.bit_depth.startswith("32"):
            sampwidth = 4
        else:
            # 12-bit export is quantised to 12-bit levels in a 16-bit container.
            sampwidth = 2
        self.wf.setsampwidth(sampwidth)
        self.wf.setnframes(self.total_frames)
        self.frames_written = 0
        self.bytes_since_flush = 0
        self.closed = False

    def write_float_chunk(self, audio: np.ndarray) -> None:
        if self.closed:
            raise RuntimeError("WAV writer is already closed")
        data = _pcm_bytes_from_float_mono(audio, self.bit_depth)
        self.wf.writeframesraw(data)
        self.frames_written += int(np.asarray(audio).size)
        self.bytes_since_flush += len(data)
        if self.bytes_since_flush >= STREAM_WAV_FLUSH_BYTES:
            self.raw.flush()
            self.bytes_since_flush = 0
...
def _prepare_morph_render_context(
    start: PatchSettings,
    end: PatchSettings,
    *,
    summary: RenderSummary,
    ...
    curve_mode: str,
    cycle_stepped: bool,
) -> tuple[int, int, float, float, float, _TableCache, list[str]]:
    """Shared setup for buffered and streaming morph renders."""
    n = max(1, int(summary.sample_count))
    sr = int(summary.sample_rate)
    freq = clamp_float(summary.frequency, MIN_FREQUENCY, MAX_FREQUENCY)
    nominal_cycles = max(1.0, summary.nominal_cycles)
    ...
    return n, sr, freq, nominal_cycles, freq / float(sr), cache, warnings


class _SequentialMorphRenderer:
    """Stateful sample renderer used by the streaming export path."""
    ...
    def __init__(
        ...
        *,
        ...
        self.cycle_stepped = bool(cycle_stepped)
        self.reverse = bool(reverse)
        self.frame_count = self.cache.frame_count
        self.phase = 0.0
        self.index = 0

    def render(self, frames: int) -> np.ndarray:
        frames = max(0, min(int(frames), self.n - self.index))
        out = np.empty(frames, dtype=np.float32)
        for j in range(frames):
            i = self.index
            raw_pos = 0.0 if self.n <= 1 else i / float(self.n - 1)
            if self.reverse:
                raw_pos = 1.0 - raw_pos

            if self.cycle_stepped:
                cycle = int(math.floor((i / float(self.sr)) * self.freq))
                if self.reverse:
                    cycle = int(max(0, round(self.nominal_cycles) - 1 - cycle))
                idx = clamp_int(cycle, 0, self.frame_count - 1)
            else:
                idx = clamp_int(int(round(raw_pos * (self.frame_count - 1))), 0, self.frame_count - 1)

            table, fm, pwm, am, fm_range_cents = self.cache.frame(idx)
            mod_sample = read_table_linear(table, self.phase)
            am_depth = clamp_float(am, 0.0, 255.0) / 255.0
            am_gain = (1.0 - am_depth) + am_depth * (0.5 + 0.5 * mod_sample)
            am_gain = clamp_float(am_gain, 0.0, 1.5)
            mod_signal = clamp_float(mod_sample * am_gain, -1.0, 1.0)
            ...
            width = clamp_float(0.5 + pwm_depth * mod_signal, 0.02, 0.98)
            if self.phase < width:
                read_phase = (self.phase / width) * 0.5
            else:
                read_phase = 0.5 + ((self.phase - width) / (1.0 - width)) * 0.5

            out[j] = read_table_linear(table, read_phase) * am_gain
            fm_scale = bounded_fm_phase_scale(mod_signal, fm, fm_range_cents)
            self.phase += min(self.base_phase_inc * fm_scale, 0.49)
            if self.phase >= 1.0:
    ...
    def notes(self) -> list[str]:
        return self.warnings + sorted(set(str(n) for n in self.cache.notes if n))[:20]


def _process_local_export_chunk(
    chunk: np.ndarray,
    *,
    dc_mode: str,
    norm_mode: str,
    ...
    """Apply cleanup that depends only on this cycle/chunk."""
    y = np.asarray(chunk, dtype=np.float64).copy()
    dc = _canonical_dc_mode(dc_mode)
    norm = _canonical_norm_mode(norm_mode)
    gain_target = clamp_float(10.0 ** (float(headroom_db) / 20.0), 0.0, 1.0)
    if dc == "Per Cycle":
        y -= float(np.mean(y)) if y.size else 0.0
    if norm == "Per Cycle Peak":
        peak = float(np.max(np.abs(y))) if y.size else 0.0
        if math.isfinite(peak) and peak > EPSILON:
            y *= gain_target / peak
    return np.clip(np.nan_to_num(y, nan=0.0, posinf=1.0, neginf=-1.0), -1.0, 1.0).astype(np.float32)
...
def _render_morph_audio_to_wav_streaming(
    wav_path: str,
    start: PatchSettings,
    end: PatchSettings,
    *,
    ...
    """Render directly to WAV when cleanup can be streamed safely.

    Returns (streamed, notes). If streamed is False, the caller should use the
    buffered render path to preserve Whole File cleanup semantics.
    """
    streamable, reason = _render_cleanup_streamable(
        dc_mode,
        norm_mode,
        samples_per_cycle=summary.samples_per_cycle,
    )
    if not streamable:
        return False, [reason]

    renderer = _SequentialMorphRenderer(
        start,
        end,
        summary=summary,
        shared_type=shared_type,
    ...
    dc = _canonical_dc_mode(dc_mode)
    norm = _canonical_norm_mode(norm_mode)
    per_cycle_cleanup = dc == "Per Cycle" or norm == "Per Cycle Peak"
    samples_per_cycle = max(float(summary.samples_per_cycle), 1.0)
    total = max(1, int(summary.sample_count))
    out_buffer = np.empty(STREAM_EXPORT_CHUNK_SAMPLES, dtype=np.float32)
    out_used = 0

    def flush_buffer(writer: _StreamingMonoWavWriter) -> None:
        nonlocal out_used
        if out_used > 0:
            writer.write_float_chunk(out_buffer[:out_used])
            out_used = 0

    with _StreamingMonoWavWriter(
        wav_path,
        sample_rate=summary.sample_rate,
        bit_depth=summary.bit_depth,
        total_frames=total,
    ) as writer:
        if per_cycle_cleanup:
            cycle_index = 0
            while renderer.index < total:
                cycle_end = int(round((cycle_index + 1) * samples_per_cycle))
                if cycle_end <= renderer.index:
                    cycle_end = renderer.index + 1
                cycle_end = min(cycle_end, total)
                segment = renderer.render(cycle_end - renderer.index)
                segment = _process_local_export_chunk(
                    segment,
                    dc_mode=dc,
                    norm_mode=norm,
                    headroom_db=headroom_db,
                ...
                while seg_pos < segment.size:
                    room = STREAM_EXPORT_CHUNK_SAMPLES - out_used
                    take = min(room, segment.size - seg_pos)
                    out_buffer[out_used:out_used + take] = segment[seg_pos:seg_pos + take]
                    out_used += take
                    seg_pos += take
                    if out_used >= STREAM_EXPORT_CHUNK_SAMPLES:
                        flush_buffer(writer)
                cycle_index += 1
        else:
            while renderer.index < total:
                frames = min(STREAM_EXPORT_CHUNK_SAMPLES, total - renderer.index)
                chunk = renderer.render(frames)
                chunk = _process_local_export_chunk(
                    chunk,
                    dc_mode=dc,
                    norm_mode=norm,
                    headroom_db=headroom_db,
                )
                writer.write_float_chunk(chunk)
        flush_buffer(writer)

    notes = renderer.notes()
    notes.append("streamed WAV export: rendered chunks were encoded and written without holding the whole audio file in RAM")
    notes.append(
        "embedded systems note: streaming reduces RAM pressure, but very long renders still stress CPU time, flash/SD writes, filesystem limits, and classic RIFF/WAV size limits"
    )
    if str(summary.bit_depth).startswith("12"):
        notes.append("12-bit export quantised to 12-bit levels inside a 16-bit PCM WAV container")
    return True, notes


def _v9_write_render_targets(self, *, directions: list[str], targets: dict[str, str], safe_prefix: str, summary: RenderSummary, raw_start: PatchSettings, raw_end: PatchSettings, start: PatchSettings, end: PatchSettings, bulk: dict | None = None) -> list[str]:
    """Write WAV+JSON targets, using streaming export when cleanup allows it."""
    created: list[str] = []
    shared_type = None if bool(self.different_types_var.get()) else self._effective_start_type()
    different_types = bool(self.different_types_var.get())
    different_fm_ranges = bool(self.different_fm_ranges_var.get())
    ...
    for direction in directions:
        ...
        streamed, stream_notes = _render_morph_audio_to_wav_streaming(
            wav_path,
            start,
            end,
            summary=summary,
        ...
        if streamed:
            ...
            notes = list(stream_notes) + list(notes)
            notes.append("buffered WAV export: Whole File cleanup or very-low-cycle-resolution cleanup required full-render processing")
            _write_wav_mono(wav_path, audio, summary.sample_rate, summary.bit_depth)
        ...
        metadata["export_writer"] = {
            "mode": "streaming" if streamed else "buffered",
            "chunk_samples": STREAM_EXPORT_CHUNK_SAMPLES if streamed else None,
            "flush_bytes": STREAM_WAV_FLUSH_BYTES if streamed else None,
            "streaming_reason": "Off/Per Cycle cleanup can be processed locally" if streamed else "Whole File or legacy cleanup requires whole-render processing",
            "embedded_systems_note": (
                "Streaming reduces RAM pressure and is friendlier to low-memory systems, but long renders still cost CPU time, storage writes, and may hit filesystem or RIFF/WAV size limits."
            ),
        }
        if str(summary.bit_depth).startswith("12"):
            metadata["bit_depth_note"] = "12-bit quantisation written in a 16-bit PCM WAV container"
...
def _v9_update_morph_consequence(self) -> None:
    try:
        summary = self._summary_from_ui()
        lines = [
            f"Output: {summary.sample_count} samples, {_format_seconds_compact(summary.duration_seconds)}, {summary.nominal_cycles:.3f} cycles at {summary.frequency:.6g} Hz.",
            f"Cycle size: {summary.samples_per_cycle:.3f} samples/cycle. Bit depth: {summary.bit_depth}.",
        ]
        try:
            dc = str(self.dc_mode_var.get())
            norm = str(self.normalise_mode_var.get())
            streamable, reason = _render_cleanup_streamable(
                dc,
                norm,
                samples_per_cycle=summary.samples_per_cycle,
            ...
            if streamable:
                lines.append(
                    "Export writer: streaming chunks to WAV for this cleanup mode; the whole audio render is not kept in RAM."
                )
            else:
                lines.append(f"Export writer: buffered path for this cleanup mode. {reason}.")
        except Exception:
            pass
        if summary.warnings:
            lines.append("Warnings: " + "; ".join(summary.warnings))
...
# Apply v9 overrides after v7, so all final export buttons and bulk export use
# chunked WAV writing wherever the selected cleanup mode permits it.
TabbedFractalApp._update_morph_consequence = _v9_update_morph_consequence
_v6_write_render_targets = _v9_write_render_targets
